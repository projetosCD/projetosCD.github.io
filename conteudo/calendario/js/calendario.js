function CalendarApp(date) {
  ...
  this.days = ['Domingo', 'Segunda-feira', 'Terça-feira', 'Quarta-feira', 'Quinta-feira', 'Sexta-feira', 'Sabado'];
  this.months = ['Jan', 'Fev', 'Mar', 'Abr', 'Mai', 'Jun', 'Jul', 'Ago', 'Set', 'Out', 'Nov', 'Dez'];
  this.quotes = ['Seja o que for que a mente do homem possa conceber e acreditar, ela pode alcançar. -Napoleon Hill',
  'Esforce-se para não ser um sucesso, mas sim para ser de valor. –Albert Einstein',
  'Duas estradas divergiram em uma floresta, e eu - eu peguei a menos percorrida, E isso fez toda a diferença.  –Robert Frost',
  'Atribuo meu sucesso a isso: nunca dei ou tomei nenhuma desculpa. –Florence Nightingale',
  'Você perde 100% das fotos que não tira. –Wayne Gretzky',
  'O mais difícil é a decisão de agir, o resto é apenas persistência. –Amelia Earhart',
  'Cada greve me aproxima do próximo "home run". –Babe Ruth',
  'Definitividade de propósito é o ponto de partida de toda conquista. –W. Clement Stone',
  'A vida não é sobre ter e ter, é dar e ser. –Kevin Kruse',
  'A vida é o que acontece com você enquanto você está ocupado fazendo outros planos. –John Lennon',
  'Nós nos tornamos o que pensamos sobre. –Earl Nightingale',
  'A vida é 10% o que acontece comigo e 90% de como eu reajo a isso. –Charles Swindoll',
  'A maneira mais comum de as pessoas desistirem de seu poder é pensar que elas não têm. –Alice Walker',
  'A mente é tudo. O que você acha que se torna.  –Buddha',
  'Ganhar não é tudo, mas querer ganhar é. –Vince Lombardi',
  'Cada criança é um artista. O problema é como se manter um artista, uma vez que ele cresce. –Pablo Picasso',
  'Você nunca pode atravessar o oceano até ter a coragem de perder de vista a costa. –Christopher Columbus',
  ...
  'Ou você toma conta do seu dia, ou o dia toma conta de você.. –Jim Rohn',
  'Se você acha que pode ou acha que não pode, você está certo. –Henry Ford',
  'Os dois dias mais importantes da sua vida são o dia em que você nasceu e o dia em que você descobre porque. –Mark Twain',
  'Tudo o que você pode fazer ou sonhar, você pode começar. Ousadia tem gênio, poder e magia nele. –Johann Wolfgang von Goethe',
  'A melhor vingança é o sucesso massivo. –Frank Sinatra',
  'As pessoas costumam dizer que a motivação não dura. Bem, nem banho. É por isso que recomendamos diariamente. –Zig Ziglar',
  'A vida diminui ou se expande em proporção à sua coragem. –Anais Nin',
  'Se você ouvir uma voz dentro de você dizer "você não pode pintar", então por todos os meios pintar e essa voz será silenciada. –Vincent Van Gogh',
  'Há apenas uma maneira de evitar críticas: não faça nada, não diga nada e não seja nada. –Aristotle',
  'Peça e será dado a você; Procure e você irá encontrar; bater e a porta será aberta para você. –Jesus',
  ...
  'Poucas coisas podem ajudar um indivíduo mais do que colocar responsabilidade sobre ele, e deixá-lo saber que você confia nele.  –Booker T. Washington'];
  this.apts = [
    {
  ...
  this.todayIsSpan.textContent = "Hoje é " + date.getDate() + " " + "de" + " " + this.months[date.getMonth()] ;  
}
...
CalendarApp.prototype.openDayWindow = function(date){
  ...
  this.dayEventsEle.textContent = _dayTopbarText + "eventos em " + this.months[day.getMonth()] + " " + day.getDate() + ", " + day.getFullYear();
